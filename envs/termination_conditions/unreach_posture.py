...
class UnreachPosture(BaseTerminationCondition):
    ...
    def get_termination(self, task, env, info={}):
        ...
        # If we reached 1500 steps, the episode is over (timeout)
        timeout_mask = check_time >= self.max_check_interval

        # For a timeout, 'done' is True (episode over), but 'bad_done' is False!
        # It didn't crash, it just ran out of time.
        done = timeout_mask
        bad_done = torch.zeros_like(done)
        exceed_time_limit = timeout_mask

        if should_log and torch.any(timeout_mask):
            self.log(f'Timeout reached, resetting!')
